        ...
        <div className="bg-white rounded-3xl w-full max-w-md border border-slate-200 shadow-2xl overflow-hidden">
          <div className="p-5 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white z-10">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-serif-heading text-lg font-bold text-slate-900">Privacy Policy</h2>
            </div>
          ...
          <div className="p-5 space-y-5">
            ...
            {SECTIONS.map((s) => (
              <div key={s.title} className="space-y-1.5">
                <h3 className="text-emerald-700 text-sm font-bold">{s.title}</h3>
                <p className="text-slate-600 text-xs leading-relaxed">{s.body}</p>
              </div>
            ))}
            <button
              onClick={onClose}
